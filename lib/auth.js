...
export async function LogIn(email, password){
  ...
  try {
    ...
    const date = new Date();
    const hour = date.setHours(date.getHours() + 1);
    const oneHour = new Date(hour).toISOString().slice(0, 19).replace('T', ' ');
  
    const LogInSession = await execute('insert into sessions(userId,sessionId,expiration) values(?,?,?)',[user.id,sessionId,oneHour]);
